...
class NavisworksAdapter(ApplicationAdapter):
    application_id = "navisworks"
    application_name = "Autodesk Navisworks"
    ...
    def identify(self) -> ActionResult:
        return ActionResult(action_id="", status="completed", started_at=datetime.now(tz=UTC), result="Navisworks adapter identified")
    ...
    def connect(self) -> ActionResult:
        ...
        return ActionResult(action_id="", status="completed", started_at=datetime.now(tz=UTC), result="connected to Navisworks")
    ...
    def get_capabilities(self) -> ApplicationCapability:
        ...
        return ApplicationCapability(
            ...
            adapter_type="navisworks",
            connection_state=state,
            capabilities=["ui_control", "document", "view", "clash", "selection"],
            supported_actions=["ui_control", "clash_run", "view_control", "selection_set"],
    ...
    def execute(self, request: ActionRequest) -> ActionResult:
        if not self._connected:
            return ActionResult(action_id=request.action_id, status="failed", started_at=datetime.now(tz=UTC), error="Not connected to Navisworks")
