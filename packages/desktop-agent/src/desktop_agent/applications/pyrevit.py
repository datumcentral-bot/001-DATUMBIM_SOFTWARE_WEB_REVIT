...
class PyRevitAdapter(ApplicationAdapter):
    application_id = "pyrevit"
    application_name = "pyRevit"
    ...
    def identify(self) -> ActionResult:
        return ActionResult(action_id="", status="completed", started_at=datetime.now(tz=UTC), result="pyRevit adapter identified")
    ...
    def connect(self) -> ActionResult:
        ...
        return ActionResult(action_id="", status="completed", started_at=datetime.now(tz=UTC), result="connected to pyRevit")
    ...
    def get_capabilities(self) -> ApplicationCapability:
        ...
        return ApplicationCapability(
            ...
            adapter_type="pyrevit",
            connection_state=state,
            capabilities=["script", "command", "extension"],
            supported_actions=["command_run", "script_execute", "script_query"],
    ...
    def execute(self, request: ActionRequest) -> ActionResult:
        if not self._connected:
            return ActionResult(action_id=request.action_id, status="failed", started_at=datetime.now(tz=UTC), error="Not connected to pyRevit")
