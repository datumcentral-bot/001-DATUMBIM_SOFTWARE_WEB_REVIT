...
class BlenderAdapter(ApplicationAdapter):
    ...
    def identify(self) -> ActionResult:
        return ActionResult(action_id="", status="completed", started_at=datetime.now(tz=UTC), result="Blender adapter identified")
    ...
    def connect(self) -> ActionResult:
        ...
        return ActionResult(action_id="", status="completed", started_at=datetime.now(tz=UTC), result="connected to Blender")
    ...
    def get_capabilities(self) -> ApplicationCapability:
        ...
        return ApplicationCapability(
            ...
            adapter_type="blender",
            connection_state=state,
            capabilities=["ui_control", "scene", "python", "file"],
            supported_actions=["scene_query", "python_execute", "file_open", "file_save"],
    ...
    def execute(self, request: ActionRequest) -> ActionResult:
        if not self._connected:
            return ActionResult(action_id=request.action_id, status="failed", started_at=datetime.now(tz=UTC), error="Not connected to Blender")
