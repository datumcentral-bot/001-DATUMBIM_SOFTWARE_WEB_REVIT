...
class DynamoAdapter(ApplicationAdapter):
    application_id = "dynamo"
    application_name = "Dynamo"
    ...
    def identify(self) -> ActionResult:
        return ActionResult(action_id="", status="completed", started_at=datetime.now(tz=UTC), result="Dynamo adapter identified")
    ...
    def connect(self) -> ActionResult:
        ...
        return ActionResult(action_id="", status="completed", started_at=datetime.now(tz=UTC), result="connected to Dynamo")
    ...
    def get_capabilities(self) -> ApplicationCapability:
        ...
        return ApplicationCapability(
            ...
            capabilities=["workspace", "script", "execution"],
            supported_actions=["workspace_open", "workspace_run", "workspace_stop", "workspace_query"],
    ...
    def execute(self, request: ActionRequest) -> ActionResult:
        if not self._connected:
            return ActionResult(action_id=request.action_id, status="failed", started_at=datetime.now(tz=UTC), error="Not connected to Dynamo")
