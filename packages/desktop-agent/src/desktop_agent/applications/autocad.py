...
class AutoCADAdapter(ApplicationAdapter):
    application_id = "autocad"
    application_name = "Autodesk AutoCAD"
    ...
    def identify(self) -> ActionResult:
        return ActionResult(action_id="", status="completed", started_at=datetime.now(tz=UTC), result="AutoCAD adapter identified")
    ...
    def connect(self) -> ActionResult:
        ...
        return ActionResult(action_id="", status="completed", started_at=datetime.now(tz=UTC), result="connected to AutoCAD")
    ...
    def get_capabilities(self) -> ApplicationCapability:
        ...
        return ApplicationCapability(
            ...
            adapter_type="autocad",
            connection_state=state,
            capabilities=["ui_control", "document", "drawing", "selection", "view"],
            supported_actions=["ui_control", "drawing_open", "drawing_save", "selection_set"],
    ...
    def execute(self, request: ActionRequest) -> ActionResult:
        if not self._connected:
            return ActionResult(action_id=request.action_id, status="failed", started_at=datetime.now(tz=UTC), error="Not connected to AutoCAD")
